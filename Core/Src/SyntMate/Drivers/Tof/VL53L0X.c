...
uint16_t 	range[NUM_MEASURES],tof_rev_id,status,measure_in_progress,measure_number;
uint32_t	the_range;

void SintMate_VL53L0X_Init(void)
{
...
void SintMate_VL53L0X_StartMeasure(void)
{
uint32_t	i;
...
{
	uint16_t	i , tmprange;
	tmprange = VL53L0X_readReg16Bit(RESULT_RANGE_STATUS + 10);
	range[measure_number-1] = ((tmprange >> 8)&0xff) | ((tmprange << 8) & 0xff00);
	VL53L0X_writeReg(SYSTEM_INTERRUPT_CLEAR, 0x01);
	measure_number--;
	if ( measure_number == 0 )
	{
